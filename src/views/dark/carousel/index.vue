  ...
  <div>
    <a-alert type="info" :show-icon="false">
      <a-space>
        ...
        <icon-notification />
        <a-typography-text
          >本广告位图片标准比例：400(宽) x 180(高)</a-typography-text
        >
      </a-space>
    </a-alert>
...
const options = reactive({
  ...
  add: {
    ...
    auth: ["dark:carousel:save"],
  },
  edit: {
    show: true,
    api: staticCarousel.update,
    auth: ["dark:carousel:update"],
  },
  delete: {
    show: true,
    api: staticCarousel.deletes,
    auth: ["dark:carousel:delete"],
    realApi: staticCarousel.realDeletes,
    realAuth: ["dark:carousel:realDeletes"],
  },
  recovery: {
    show: true,
    api: staticCarousel.recoverys,
    auth: ["dark:carousel:recovery"],
  },
...
<script>
export default { name: "dark:carousel" };
</script>