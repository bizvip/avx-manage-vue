  ...
  <div>
    <a-alert type="info" :show-icon="false">
      <a-space>
        ...
        <icon-notification />
        <a-typography-text
          >目前只提供1个顶部横幅广告位，所以无论添加多少，只展示最新添加、且处在允许展示时间段内的一张
        </a-typography-text>
        <icon-notification />
        <a-typography-text
...
const options = reactive({
  ...
  add: {
    ...
    auth: ["dark:banner:save"],
  },
  edit: {
    show: true,
    api: staticBanner.update,
    auth: ["dark:banner:update"],
  },
  delete: {
    show: true,
    api: staticBanner.deletes,
    auth: ["dark:banner:delete"],
    realApi: staticBanner.realDeletes,
    realAuth: ["dark:banner:realDeletes"],
  },
  recovery: {
    show: true,
    api: staticBanner.recoverys,
    auth: ["dark:banner:recovery"],
  },
...
<script>
export default { name: "dark:banner" };
</script>