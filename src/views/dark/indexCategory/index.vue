<template>
  <div>
    <a-alert type="info" :show-icon="false">
      <a-space>
        <a-typography-text bold underline>暗网导航</a-typography-text>
        >
...
const siteId = import.meta.env.VITE_APP_DARK_ID;
...
const options = reactive({
  ...
  add: {
    ...
    auth: ["dark:indexCategory:save"],
  },
  edit: {
    show: true,
    api: staticIndexCategory.update,
    auth: ["dark:indexCategory:update"],
  },
  delete: {
    show: true,
    api: staticIndexCategory.deletes,
    auth: ["dark:indexCategory:delete"],
    realApi: staticIndexCategory.realDeletes,
    realAuth: ["dark:indexCategory:realDeletes"],
  },
  recovery: {
    show: true,
    api: staticIndexCategory.recoverys,
    auth: ["dark:indexCategory:recovery"],
  },
...
<script>
export default { name: "dark:indexCategory" };
</script>