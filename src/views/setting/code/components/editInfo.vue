        ...
        <a-tabs v-model:active-key="activeTab">
          <a-tab-pane title="配置信息" key="base_config">
            ...
            <a-row :gutter="24">
              ...
              <a-col :xs="24" :md="12" :xl="12">
                ...
                >
                  <a-radio-group
                    v-model:model-value="form.generate_type"
                    type="button"
                  >
            ...
            <a-row :gutter="24">
              ...
              <a-col :xs="24" :md="12" :xl="12">
                ...
                >
                  <a-radio-group
                    v-model:model-value="form.build_menu"
                    type="button"
                  >
          ...
          <a-tab-pane title="字段配置" key="field_config">
            <a-alert title="提示">
              ...
              设置相应字段为
              <a-tag color="#168cff">array</a-tag>
              类型
              <p>
          ...
          <a-tab-pane title="菜单配置" key="menu_config">
            <a-alert title="提示"
              >未选择的菜单，后端也对应不生成方法。注意：列表按钮菜单是默认的</a-alert
            >
            <a-checkbox-group
              direction="vertical"
          ...
          <a-tab-pane title="关联配置" key="relation_config">
            <a-alert title="提示"
              >模型关联支持：一对一、一对多、一对多（反向）、多对多。</a-alert
            >
            ...
            <div v-for="(item, index) in formOptions.relations" :key="index">
              ...
              <a-row :gutter="24">
                ...
                <a-col :xs="24" :md="12" :xl="12">
                  <a-form-item
                    :label="
                      item.type === 'belongsToMany' ? '中间表外键' : '外键'
                    "
                    field="foreignKey"
                    label-col-flex="auto"
                    :label-col-style="{ width: '100px' }"
...
const init = () => {
  ...
  // 请求所有数据表
  dataMaintain
    .getPageList({ pageSize: 999 })
    .then((res) => (tables.value = res.data.items));
  // 请求所有字典类型
  dictType
    .getTypeList({ pageSize: 999 })
    .then((res) => (dicts.value = res.data.items));
};
