      ...
      <template #new_name="{ record }">
        <a-input-group>
          ...
          <a-tooltip content="复制表名称到新名称">
            <a-link @click="() => (newName[record.name] = record.name)">
              <icon-copy />
            </a-link>
          </a-tooltip>
        </a-input-group>
      ...
      <template #new_comment="{ record }">
        <a-input-group>
          ...
          <a-tooltip content="复制表注释到新注释">
            <a-link @click="() => (newComment[record.name] = record.comment)">
              <icon-copy />
            </a-link>
          </a-tooltip>
        </a-input-group>
...
const switchSource = () => {
  ...
  const newCommentColumn = columns.find(
    (item) => item.dataIndex === "new_comment",
  );
  ...
  crud.api =
    sourceName.value === "MineAdmin"
      ? dataMaintain.getPageList
      : dataSource.getDataSourceTablePageList;
  crudRef.value.requestParams["id"] =
    sourceName.value === "MineAdmin" ? undefined : sourceName.value;
