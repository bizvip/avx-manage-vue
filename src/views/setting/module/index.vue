<template>
  <div class="ma-content-block lg:flex justify-between pt-4 pl-2 pr-2 pb-2">
    ...
    <ma-crud :options="crud" :columns="columns" ref="crudRef">
      <template #operationCell="{ record }">
        <a-popconfirm
          ...
          position="bottom"
        >
          <a-link v-auth="['setting:module:install']">
            <icon-record />
            安装数据
          </a-link>
        </a-popconfirm>
        ...
        <a-popconfirm
          ...
          :content="`确定要${record.enabled ? '停用' : '启用'}${
            record.label
          }吗?`"
          position="bottom"
        >
          <a-link v-auth="['setting:module:status']" v-if="record.enabled">
            <icon-info-circle />
            停用
          </a-link>
          <a-link v-auth="['setting:module:status']" v-else>
            <icon-check-circle />
            启用
          </a-link>
        </a-popconfirm>
    ...
    >
      ...
      <div>
        此操作会删除文件和数据表，如果执行请在下面输入框输入：<span
          class="text-red-500"
          >{{ currentModule.name }}</span
        >
      </div>
      <a-input
        :placeholder="`请输入 ${currentModule.name}`"
        class="mt-2"
        v-model="name"
      />
    </a-modal>
...
const switchEnbaled = async (record) => {
  ...
  const response = await module.modifyStatus({
    name: record.name,
