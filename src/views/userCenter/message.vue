<template>
  <div
    class="ma-content-block p-3 lg:h-full block lg:border-0 lg:flex justify-between"
  >
    <ul class="w-full lg:w-52 msg-menu p-2 shadow" ref="msgMenuRef">
      ...
      >
        <Component
          :is="typeIcon[item.key] ? typeIcon[item.key] : 'icon-message'"
        />
      ...
      <a-spin :loading="detailLoading" tip="数据加载中..." class="block">
        <a-typography :style="{ marginTop: '-30px' }">
          ...
          <a-typography-paragraph
            class="text-right"
            style="font-size: 13px; color: var(--color-text-3)"
          >
            <a-space size="large">
...
const loadData = (key) => {
  crud.value.api =
    key === "send_box" ? queueMessage.getSendList : queueMessage.getReceiveList;
  const sendBy = {
    title: "发送人",
...
const crud = ref({
  ...
  add: {
    show: true,
    text: "发私信",
    api: queueMessage.sendPrivateMessage,
  },
  delete: {
    show: true,
    api: queueMessage.deletes,
  },
