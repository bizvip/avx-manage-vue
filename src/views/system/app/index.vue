  ...
  <div class="ma-content-block lg:flex justify-between pt-4 pl-2 pr-2 pb-2">
    ...
    <ma-crud :options="crud" :columns="columns" ref="crudRef">
      ...
      <template #operationBeforeExtend="{ record }" v-if="!isRecovery">
        <a-link @click="openDoc(record)">
          <icon-book />
          文档
        </a-link>
        <a-link @click="bindApi(record.id)">
          <icon-pushpin />
          绑定
        </a-link>
      </template>
...
const changeStatus = async (status, id) => {
  const response = await app.changeStatus({
    id,
    status,
  });
  if (response.success) {
...
const crud = reactive({
  ...
  add: {
    show: true,
    api: app.save,
    auth: ["system:app:save"],
  },
  edit: {
    show: true,
    api: app.update,
    auth: ["system:app:update"],
  },
  delete: {
    show: true,
    api: app.deletes,
    ...
    layout: [
      {
        ...
        cols: [
          {
            span: 12,
            formList: [{ dataIndex: "group_id" }],
          },
          {
            span: 12,
            formList: [{ dataIndex: "app_name" }],
          },
        ],
      },
      {
        ...
        cols: [
          {
            span: 18,
            formList: [{ dataIndex: "app_id" }],
          },
          {
            span: 5,
            offset: 1,
            formList: [{ dataIndex: "app_id_button" }],
          },
        ],
      },
      {
        ...
        cols: [
          {
            span: 18,
            formList: [{ dataIndex: "app_secret" }],
          },
          {
            span: 5,
            offset: 1,
            formList: [{ dataIndex: "app_secret_button" }],
          },
        ],
      },
    ],
