    ...
    <a-drawer v-model:visible="visible" width="600px" :footer="false">
      ...
      >
        <a-collapse-item header="队列数据" key="request">
          <ma-code-editor
            v-model="currentRow.queue_content"
            is-bind
            :height="150"
            readonly
          />
        ...
        <a-collapse-item header="队列日志" key="response">
          <pre>{{
            currentRow.log_content == null
              ? "无日志数据"
              : currentRow.log_content
          }}</pre>
        </a-collapse-item>
      </a-collapse>
    </a-drawer>
...
const crud = reactive({
  ...
  delete: {
    show: true,
    api: queueLog.deletes,
    auth: ["system:queueLog:delete"],
  },
});
