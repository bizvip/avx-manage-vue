<template>
  <a-modal v-model:visible="visible" fullscreen :footer="false">
    <template #title>
      维护 {{ currentRow.name }} 的{{
        currentType == "request" ? "请求参数" : "响应参数"
      }}
    </template>
    <!-- CRUD 组件 -->
...
const changeStatus = async (status, id) => {
  const response = await apiColumn.changeStatus({
    id,
    status,
  });
  if (response.success) {
...
const crud = reactive({
  ...
  add: {
    show: true,
    api: apiColumn.save,
    auth: ["system:api:save"],
  },
  edit: {
    show: true,
    api: apiColumn.update,
    auth: ["system:api:update"],
  },
  delete: {
    show: true,
  ...
  recovery: {
    show: true,
    api: apiColumn.recoverys,
    auth: ["system:api:recovery"],
  },
