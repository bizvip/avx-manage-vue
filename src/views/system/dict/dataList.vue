...
const changeStatus = async (status, id) => {
  const response = await dict.changeStatus({
    id,
    status,
  });
  if (response.success) {
...
const changeSort = async (value, id) => {
  const response = await dict.numberOperation({
    id,
    numberName: "sort",
    numberValue: value,
  });
...
const crud = reactive({
  ...
  add: {
    show: true,
    api: dict.saveDictData,
    auth: ["system:dict:save"],
  },
  edit: {
    show: true,
    api: dict.updateDictData,
  ...
  recovery: {
    show: true,
    api: dict.recoverysDictData,
    auth: ["system:dict:recovery"],
  },
