  ...
  <a-layout-content>
    <div class="ma-content-block lg:flex p-4">
      <div class="flex justify-between w-full redis-info mt-3">
        <a-descriptions
          ...
          bordered
          title="Redis信息"
          class="lg:w-10/12 w-full"
        >
          <a-descriptions-item label="Redis版本">{{
            server.version
          }}</a-descriptions-item>
          <a-descriptions-item label="客户端连接数">{{
            server.clients
          }}</a-descriptions-item>
          ...
          <a-descriptions-item label="AOF状态">{{
            server.aof_enabled
          }}</a-descriptions-item>
          <a-descriptions-item label="已过期key">{{
            server.expired_keys
          }}</a-descriptions-item>
          <a-descriptions-item label="系统使用key">{{
            server.sys_total_keys
          }}</a-descriptions-item>
        </a-descriptions>
    ...
    <div class="ma-content-block p-4 mt-3">
      ...
      <div class="mt-5 lg:flex justify-between">
        ...
        <a-textarea
          v-model="content"
          class="lg:w-4/12 w-full lg:ml-5 mt-3 lg:mt-0"
          readonly
        />
      </div>
    </div>
