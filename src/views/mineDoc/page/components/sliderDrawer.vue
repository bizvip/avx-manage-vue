        ...
        <div class="pl-2 pr-2">
          ...
          <a-descriptions :column="1" bordered>
            <a-descriptions-item label="接口地址"
              >/api/v1/{{ row.access_name }}</a-descriptions-item
            >
            <a-descriptions-item label="认证模式">
              <a-tag v-if="row.auth_mode === 1" color="green" bordered
                >简易模式</a-tag
              >
            ...
            <a-descriptions-item label="认证说明">
              <div v-if="row.auth_mode === 1">
                简易模式下，只需要在接口后面带上
                <a-tag color="orangered">app_id</a-tag>
                和
                <a-tag color="orangered">identity</a-tag>
              </div>
              <div v-else>
                复杂模式下，先需要获取
                <a-tag color="blue">AccessToken</a-tag>
                ，再以此请求接口
              </div>
            </a-descriptions-item>
          </a-descriptions>
          ...
          <a-table :data="requestData" :columns="requestCols">
            <template #dataType="{ record }">
              <template v-for="dic in dictData" :key="dic.id">
                <a-tag color="blue" v-if="dic.key == record.data_type">{{
                  dic.title
                }}</a-tag>
              </template>
            </template>
