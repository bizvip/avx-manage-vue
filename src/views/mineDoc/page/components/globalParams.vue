  ...
  >
    ...
    <a-tabs type="rounded" v-model:active-key="activeTab">
      <a-tab-pane key="query" title="设置全局Query">
        <div class="pl-2 pr-2">
          <a-button type="primary" class="mb-3" @click="add">
            <icon-plus />
            新增
          </a-button>
...
const add = () => {
  globalParams[activeTab.value].push({
    name: "",
    value: "",
  });
};
  ...
  if (docStore.globalParams) {
    ...
    for (let name in docStore.globalParams.header) {
      globalParams.header.push({
        name: name,
        value: docStore.globalParams.header[name],
      });
    }
    for (let name in docStore.globalParams.query) {
      globalParams.query.push({
        name: name,
        value: docStore.globalParams.query[name],
      });
    }
    for (let name in docStore.globalParams.body) {
      globalParams.body.push({
        name: name,
        value: docStore.globalParams.body[name],
      });
    }
  }
};
