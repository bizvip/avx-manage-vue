  ...
  >
    <a-layout-header class="nav flex justify-between p-2">
      <div class="flex text-2xl">
        <img src="/logo.svg" width="54" class="mr-2" />
        <span class="mt-1">API Document</span>
      </div>
      <a-space>
        <div
    ...
    <a-layout-footer
      class="footer"
      :style="`background-color: ${
        appStore.mode == 'light' ? '#fff' : '#242424'
      }`"
    >
      IntegratedGroupOfWebsites v{{ config.version }} release Copyright ©
      2005-{{ dayjs().format("YYYY") }} MAYA Corp. All Rights Reserved.
    </a-layout-footer>
  </a-layout>
</template>
